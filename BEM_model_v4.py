...
# local blade angle theta = twist + pitch
...
def loadBladeElement(vnorm, vtan, chord, theta_deg, polar_alpha, polar_cl, polar_cd):
    ...
    Conventions used here for a wind turbine:
    - theta_deg = local blade angle = twist + pitch
    - phi_deg   = inflow angle = atan(Uax / Utan)
    - alpha_deg = phi_deg - theta_deg

    With this convention, positive alpha should normally give positive lift
    for the turbine operating case.
    """
    ...
    # Force components in rotor-normal and rotor-tangential directions
...
def solveStreamtube(
    ...
    Sign convention:
    - dT > 0 means thrust in wind-turbine direction
    - dQ > 0 means extracted turbine torque
    """
    ...
    for _ in range(max_iter):
        ...
        # Minus sign: convert aerodynamic tangential force on blade
        # to extracted shaft torque for a wind turbine
        ...
        # Use extracted torque sign consistently in tangential induction update
        denom = 4 * np.pi * rho * Uinf * (1 - a_next) * Omega * rmid**2
        aline_new = dQ / (denom * dr)
    ...
    Cq_local = dQ / (0.5 * rho * Uinf**2 * Radius * chord * dr * NBlades)
...
def plot_spanwise_comparison(result_with, result_without, tsr):
    ...
    axs[1, 1].set_ylabel("torque-producing tangential load [N/m]")
    axs[1, 1].grid()
    axs[1, 1].legend()
