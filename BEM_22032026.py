...
def visualiser(results, Uinf, Radius, tsr):
    ...
    plt.title(rf'Normal and tagential force, non-dimensioned by $\frac{{1}}{{2}} \rho U_\infty^2 R$ (TSR = {tsr})')
...
def executeBEM(Uinf, TSR, RootLocation_R, TipLocation_R , Omega, Radius, NBlades, r_R, chord_distribution, twist_distribution, polar_alpha, polar_cl, polar_cd, plot_results=True):
    ...
    if visualise == True and plot_results:
...
def influence_annuli():
    ...
    for i in range(len(elements)):
        ...
        CT, CP, results, Thrust, Torque, J = executeBEM(U0, TSR, RootLocation_R, TipLocation_R,
            Omega[0], Radius, blades, r_R, chord_distribution, twist_distribution, polar_alpha, polar_cl, polar_cd, plot_results=False)
...
def main():
    ...
    for j in range(len(TSR)):
        ...
        Omega[j], Radius, blades, r_R, chord_distribution, twist_distribution, polar_alpha, polar_cl, polar_cd, plot_results=True)
    ...
    if visualise == True:
        ...
        plt.ylabel('Load')
        plt.title('Rotor loads vs Advance Ratio (all TSR cases)')
        plt.grid()
        plt.legend()
        plt.show()
